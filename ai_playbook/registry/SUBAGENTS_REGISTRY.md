...
> ⚠️ **Claude agents は `_compiled` を参照**しています。`~/.claude/agents/` を直接編集せず、SSOT の `subagents/` を編集して `install.ps1` で再ビルドしてください。
...
## Extended エージェント（追加2体）

> **これらは `-Mode extended` で有効化されます。**
> minimal の3体に加えて、PoC/テスト専任の2体を追加した推奨セット。

---

### 4. rapid-prototyper（高速プロトタイピング）
| 項目 | 内容 |
|------|------|
| **パス** | [subagents/extended/rapid-prototyper.md](../subagents/extended/rapid-prototyper.md) |
| **責務** | MVP/PoC の高速実装、最小動作確認、技術検証 |
| **モード** | `extended` で有効 |
| **呼ぶとき** | 新規PoCを素早く作りたい、技術検証したい |
| **呼ばないとき** | 本番品質が求められる場合、計画が確定していない |

#### 呼び出し例
```
@rapid-prototyper このAPIを使ったPoCを最小構成で作って
```

---

### 5. test-writer-fixer（テスト専任）
| 項目 | 内容 |
|------|------|
| **パス** | [subagents/extended/test-writer-fixer.md](../subagents/extended/test-writer-fixer.md) |
| **責務** | テスト追加、テスト修正、カバレッジ改善 |
| **モード** | `extended` で有効 |
| **呼ぶとき** | テストを追加したい、失敗テストを修正したい |
| **呼ばないとき** | テスト対象のコードが未確定 |

#### 呼び出し例
```
@test-writer-fixer この機能のユニットテストを追加して
```
...
> **catalog は `-Mode catalog` で有効化されます（default OFF）。**
> 37体以上のエージェントを含むため、導入は「カテゴリを1つずつ」が推奨。

| カテゴリ | エージェント例 |
|----------|----------------|
| engineering | ai-engineer, backend-architect, devops-automator... |
| design | ui-designer, ux-researcher, brand-guardian... |
| marketing | content-creator, growth-hacker, twitter-engager... |
| testing | api-tester, performance-benchmarker... |
| product | sprint-prioritizer, feedback-synthesizer... |

詳細は `subagents/catalog/` を参照。
